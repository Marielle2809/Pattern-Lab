	...
	protected function gatherPatternInfo() {
		...
		foreach($patternObjects as $name => $object) {
			...
			if ($object->isDir() && ($depth == 0)) {
				...
				// set-up the names
				$patternSubtype      = $object->getFilename();                              // 02-text
				...
				if ($patternSubtypeSet) {
					$patternPath     = $patternType.$dirSep.$patternSubtype.$dirSep.$pattern; // 00-base/01-breakpoints/00-colors
					$patternPathDash = str_replace($dirSep,"-",$patternPath);                 // 00-base-01-breakpoints-00-colors (file path)
				} else {
					$patternPath     = $patternType.$dirSep.$pattern;                         // 00-base/00-colors
					$patternPathDash = str_replace($dirSep,"-",$patternPath);                 // 00-base-00-colors (file path)
				}
		...
		$patternHeadPath    = __DIR__.$this->sp."00-base/00-rythm/_00-head.mustache";
		$patternFootPath    = __DIR__.$this->sp."00-base/00-rythm/_01-foot.mustache";
